...
const INPUT_DIR = path.join(ROOT_DIR, "src", "clientuat", "leo");
const OUTPUT_DIR = path.join(ROOT_DIR, "src", "clientuat", "leo", "output");
...
if (files.length === 0) {
  console.error("No input files found in clientuat/leo");
  process.exit(1);
}
