...
const INPUT_DIR = path.join(ROOT_DIR, "src", "clientuat", "shionogi");
const OUTPUT_DIR = path.join(ROOT_DIR, "src", "clientuat", "shionogi", "output");
...
if (files.length === 0) {
  console.error("No input files found in clientuat/shionogi");
  process.exit(1);
}
