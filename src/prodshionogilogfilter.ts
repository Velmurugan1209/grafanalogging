...
const INPUT_DIR = path.join(ROOT_DIR, "src", "prod", "shionogi");
const OUTPUT_DIR = path.join(ROOT_DIR, "src", "prod", "shionogi", "output");
...
if (files.length === 0) {
  console.error("No input files found in prod/shionogi");
  process.exit(1);
}
...
const labelEntries: Record<string, string>[] = [];

for (const series of data.series ?? []) {
  for (const field of series.fields ?? []) {
    if (field.name === "labels" && field.values) {
      labelEntries.push(...field.values);
    }
  }
}

if (labelEntries.length === 0 && data.data?.values?.[0]) {
  labelEntries.push(...data.data.values[0]);
}

for (const value of labelEntries) {
      totalLogs++;
